...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            //Substitute previous assert() with RangeProof for Puzzle
            puzzleRangeProof[i][j] = RangeProof(4);
            puzzleRangeProof[i][j].range[0] <== 0;
            puzzleRangeProof[i][j].range[1] <== 9;
            puzzleRangeProof[i][j].in <== i;
            assert(puzzleRangeProof[i][j].out == 1); //Garantee range between 0 and 9
            //Substitute previous assert() with RangeProof for Solution
            solutionRangeProof[i][j] = RangeProof(4);
            solutionRangeProof[i][j].range[0] <== 0;
            solutionRangeProof[i][j].range[1] <== 9;
            solutionRangeProof[i][j].in <== i;
            assert(solutionRangeProof[i][j].out == 1); //Garantee range between 0 and 9
